...
SET @sql = IF(
  ...
  "ALTER TABLE users ADD COLUMN created_by BIGINT UNSIGNED NULL AFTER is_active",
  "SELECT 1"
);
...
SET @sql = IF(
  ...
  "ALTER TABLE users ADD INDEX idx_users_created_by (created_by)",
  "SELECT 1"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = IF(
  ...
  "ALTER TABLE transactions ADD INDEX idx_transactions_code (code)",
  "SELECT 1"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = IF(
  ...
  "ALTER TABLE transactions ADD INDEX idx_transactions_toko_status_created_at (toko_id, status, created_at)",
  "SELECT 1"
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
